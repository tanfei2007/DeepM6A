...
print 'loading data'
file_name = '../../seq/hdf5/30/m6A_30'
idx = sys.argv[1]
print idx
trainmat = h5py.File(file_name + '.train_' + str(idx) + '.hdf5', 'r')
validmat = h5py.File(file_name + '.valid_' + str(idx) + '.hdf5', 'r')
testmat = h5py.File(file_name + '.test_' + str(idx) + '.hdf5', 'r')
...
model.add(Dense(input_dim=X_train.shape[1], output_dim=200, init='he_normal'))
...
model.add(Dense(output_dim=150, init='he_normal'))
#model.add(Activation('relu'))
model.add(LeakyReLU(alpha=.001))
model.add(Dropout(0.5))
...
bestmodel = './bestmodel' + str(idx) + '.hdf5'
...
Hist = model.fit(X_train, y_train, batch_size=256, nb_epoch=500, shuffle=True, verbose=2, 
...
df.to_csv('./rslt/rslt_Ara_len30index_' + str(idx) + '.csv', index = False)
